...
// Get books by authors with 't' in their name
function BooksWithTInAuthorName(books) {
    let tBooks = [];
    for (let book of books) {
...
// Get all ISBNs for a given author
...
// Sort books by title
function sortBooksTitle(books, ascending = true) {
...
// Sort books by year
function sortBooksYear(books, ascending = true) {
    let sortedBooks = [...books];
    sortedBooks.sort((a, b) => {
        if (ascending) {
...
// Group books by author last name
function ByLastName(books) {
    let groups = {};
    for (let book of books) {
        let lastName = book.author.split(' ').pop();
        if (!groups[lastName]) {
            groups[lastName] = [];
        }
        groups[lastName].push(book);
    }
    return groups;
}

// Group books by author first name
function ByFirstName(books) {
    let groups = {};
    for (let book of books) {
        ...
        groups[firstName].push(book);
    }
    return groups;
}

const tests = test("My books they are a mess!");
...
tests.isEqual(BooksStartingWithThe(testBooks).length, 34, "Should find 34 books starting with 'The'");
tests.isEqual(BooksWithTInAuthorName(testBooks).length, 51, "Should find 51 books with 't' in author name");
tests.isEqual(countBooksAfter1992(testBooks), 95, "Should find 95 books after 1992");
tests.isEqual(countBooksBefore2004(testBooks), 44, "Should find 44 books before 2004");
tests.isEqual(getIsbnsByAuthor(testBooks, "Terry Pratchett").length, 41, "Should find 13 ISBNs for Terry Pratchett");
tests.isEqual(sortBooksTitle(testBooks)[0].title, "A Hat Full of Sky", "Should sort books by title correctly");
tests.isEqual(sortBooksYear(testBooks)[0].publication_year, 1854, "Should sort books by year correctly");
tests.isEqual(Object.keys(ByLastName(testBooks)).length, 16, "Should group books by author last name");
tests.isEqual(Object.keys(ByFirstName(testBooks)).length, 15,"Should group books by author first name");
